...
describe("generateAlternates", () => {
  it("should generate canonical URL without locale prefix", () => {
    const result = generateAlternates("/about");
    expect(result.canonical).toBe(`${seoConfig.siteUrl}/about`);
  });

  it("should handle root pathname correctly", () => {
    const result = generateAlternates("");
    expect(result.canonical).toBe(`${seoConfig.siteUrl}/`);
  });

  it("should handle slash pathname correctly", () => {
    const result = generateAlternates("/");
    expect(result.canonical).toBe(`${seoConfig.siteUrl}/`);
  });
});
...
describe("generateOgMetadata", () => {
  ...
  it("should construct URL with pathname", () => {
    ...
    expect(result?.url).toBe(`${seoConfig.siteUrl}/about`);
  });
});
